...
test("renderMarkdown handles multiple links", () => {
  const markdown =
    "Check [first link](https://first.com) and [second link](https://second.com)";
  const result = renderMarkdown(markdown);
  assert.ok(
    result.includes(
      '<a target="_blank" href="https://first.com">first link</a>'
    ) &&
      result.includes(
        '<a target="_blank" href="https://second.com">second link</a>'
      )
  );
});
...
test("renderMarkdown preserves link title attribute", () => {
  ...
  assert.equal(
    result,
    '<p><a target="_blank" href="https://example.com" title="Title">Link with title</a></p>'
  );
});
